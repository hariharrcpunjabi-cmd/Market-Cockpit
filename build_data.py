...
# ------------------------------------------------------------------ DATA FRESHNESS
def compute_freshness(as_of, current_stocks):
    """How stale is this run?

    Answers two questions the dashboard could not answer before:
      1. How many days since the previous data refresh.
      2. Whether this run is a genuine refresh or a re-run on an unchanged CSV.

    (2) matters because a scheduled Action fires whether or not new CSVs were
    uploaded. A re-run on a stale CSV writes a duplicate snapshot that looks
    like a real week and silently corrupts the tracker and ledger.
    """
    import glob, datetime
    prev_date, prev_closes = None, {}
    for sp in sorted(glob.glob("history/stage2-*.json")):
        d0 = sp.split("stage2-")[-1].replace(".json", "")
        if d0 >= as_of:
            continue
        try:
            d = json.load(open(sp))
            stocks = d.get("scanners", {}).get("trading", {}).get("stocks", [])
            prev_date = d.get("as_of") or d0
            prev_closes = {s["symbol"]: s.get("close") for s in stocks if s.get("symbol")}
        except Exception:
            pass

    cur_closes = {s["symbol"]: s.get("close") for s in (current_stocks or []) if s.get("symbol")}
    days_since = None
    if prev_date:
        try:
            days_since = (datetime.date.fromisoformat(as_of)
                          - datetime.date.fromisoformat(prev_date)).days
        except Exception:
            pass

    identical = bool(prev_closes) and cur_closes == prev_closes
    if identical:
        status, msg = "stale", "Same prices as the previous run \u2014 the CSVs were not refreshed."
    elif days_since is None:
        status, msg = "first", "First run. Comparisons start from the next refresh."
    elif days_since > 10:
        status, msg = "aging", f"{days_since} days since the last refresh."
    else:
        status, msg = "fresh", f"Updated {days_since} day{'s' if days_since != 1 else ''} ago."
    return {"as_of": as_of, "prev_date": prev_date, "days_since": days_since,
            "status": status, "message": msg, "stale_csv": identical}
...
def main():
    ...
    stage2["freshness"] = compute_freshness(as_of, scanners.get("trading", {}).get("stocks", []))
    ...
    fr = stage2["freshness"]
    print(f"  freshness: {fr['status']} \u2014 {fr['message']}")
    if fr["stale_csv"]:
        print("  *** WARNING: CSVs unchanged since last run. This snapshot is a duplicate. ***")
